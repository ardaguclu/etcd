...
func ctlV3MemberRemove(cx ctlCtx, ep, memberID, clusterID string) error {
	...
	return spawnWithExpect(cmdArgs, cx.envMap, fmt.Sprintf("%s removed from cluster %s", memberID, clusterID))
}
...
func ctlV3MemberAdd(cx ctlCtx, peerURL string, isLearner bool) error {
	...
	return spawnWithExpect(cmdArgs, cx.envMap, " added to cluster ")
}
...
func ctlV3MemberUpdate(cx ctlCtx, memberID, peerURL string) error {
	...
	return spawnWithExpect(cmdArgs, cx.envMap, " updated in cluster ")
}
