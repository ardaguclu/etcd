...
func ctlV3AuthEnable(cx ctlCtx) error {
	...
	return spawnWithExpect(cmdArgs, cx.envMap, "Authentication Enabled")
}
...
func ctlV3AuthDisable(cx ctlCtx) error {
	...
	return spawnWithExpect(cmdArgs, cx.envMap, "Authentication Disabled")
}
...
func authStatusTest(cx ctlCtx) {
	...
	if err := spawnWithExpect(cmdArgs, cx.envMap, "enabled"); err != nil {
		cx.t.Fatal(err)
	}
	if err := spawnWithExpect(cmdArgs, cx.envMap, "authRevision"); err != nil {
		cx.t.Fatal(err)
	}
}
...
func ctlV3PutFailAuth(cx ctlCtx, key, val string) error {
	return spawnWithExpect(append(cx.PrefixArgs(), "put", key, val), cx.envMap, "authentication failed")
}

func ctlV3PutFailPerm(cx ctlCtx, key, val string) error {
	return spawnWithExpect(append(cx.PrefixArgs(), "put", key, val), cx.envMap, "permission denied")
}
...
func authSetupTestUser(cx ctlCtx) {
	...
	if err := spawnWithExpect(append(cx.PrefixArgs(), "role", "add", "test-role"), cx.envMap, "Role test-role created"); err != nil {
		cx.t.Fatal(err)
	}
	...
	if err := spawnWithExpect(cmd, cx.envMap, "Role test-role updated"); err != nil {
		cx.t.Fatal(err)
	}
}
...
func authTestRoleList(cx ctlCtx) {
	...
	cx.user, cx.pass = "root", "root"
	authSetupTestUser(cx)
	if err := spawnWithExpect(append(cx.PrefixArgs(), "role", "list"), cx.envMap, "test-role"); err != nil {
		cx.t.Fatal(err)
	}
}
...
func certCNAndUsername(cx ctlCtx, noPassword bool) {
	...
	if err := spawnWithExpect(append(cx.PrefixArgs(), "role", "add", "test-role-cn"), cx.envMap, "Role test-role-cn created"); err != nil {
		cx.t.Fatal(err)
	}
