...
func getFormatTest(cx ctlCtx) {
	...
	for i, tt := range tests {
		...
		if err := spawnWithExpect(cmdArgs, cx.envMap, tt.wstr); err != nil {
...
func getKeysOnlyTest(cx ctlCtx) {
	...
	if err := spawnWithExpect(cmdArgs, cx.envMap, "key"); err != nil {
		cx.t.Fatal(err)
	}
...
func ctlV3Put(cx ctlCtx, key, value, leaseID string, flags ...string) error {
	...
	return spawnWithExpect(cmdArgs, cx.envMap, "OK")
}
